...
export async function generateEmbeddingsInPineconeVectorStore(docId: string) {
    ...
    if (namespaceAlreadyExists) {
        ...
        // generate the embeddings and store them in the Pincone vector store
        const splitDocs = await generateDocs(docId);

        console.log(`--- Storing the embeddings in namespace ${docId} in the ${indexName} Pinecone vector stroe... ---`);
...
const generateLangchainCompletion = async (docId: string, question: string) => {
    ...
    // Create a retriever to search through the vector strore
    console.log("--- Creating a retriever... ___");
    ...
    const historyAwarePrompt = ChatPromptTemplate.fromMessages([
        ...
        [
            "user",
            "Given the above conversation, geberate a search query to look up in order to get information relevant to the conversation",
        ],
    ]);
